...
export default class Exchange {
  ...
  getAllExchangeServices() {
    return this.services.OvhHttp
      ...
      .then((serviceIds) => this.services.$q.all(map(serviceIds, (serviceId) =>
          this.getExchangeServices(serviceId)
        )),
      )
      .then((services) => reduce(services, (flattened, other) => {
  ...
  getExchangeServices(organizationName) {
    return this.services.iceberg('/email/exchange/:organizationName/service')
      .query()
      ...
      .then((services) =>
        filter(
          services,
          (service) =>
            has(service, 'displayName') && has(service, 'offer'),
        ),
      )
      .then((services) =>
   ...
   */
  getAccounts(exchange, pageSize, offset, search, configurableOnly, type, timeout) {
